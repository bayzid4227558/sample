...
class StudentDataStorage:
    ...
    def store_student_data(self, data):
        cleaned_data = f"{datetime.now()} -> {data}"
    ...
    def take_student_data_input(self):
        while True:
            order_number = input("Please insert your order number (type 'exit' to return to the menu): ")
            if order_number == 'exit':
                self.store_student_data(self.student_array)
                break
            student_name = input("Please insert a student name: ")
            self.student_array.append({order_number: student_name})
...
if __name__ == "__main__":
    student_data_storage = StudentDataStorage()
    student_data_storage.user_input_function()
